...
app.post('/api/payment/create-order', auth, async (req, res) => {
  try {
    ...
    console.log("Using Razorpay Credentials:", {
      ...
      hasTrailingSpaceId: process.env.RAZORPAY_KEY_ID?.endsWith(' '),
      hasTrailingSpaceSecret: process.env.RAZORPAY_KEY_SECRET?.endsWith(' ')
    });
    ...
    res.json({
      order,
      keyId: process.env.RAZORPAY_KEY_ID // send key to frontend securely
    });
  } catch (error) {
